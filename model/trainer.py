...
class TeamRatingModel:
    ...
    def predict(self, home_team: str, away_team: str, league: str = "") -> dict:
        ...
        away_xg = (
            ar["off"] * hr["def"] * (2 - self.home_advantage) if self.home_advantage > 1 else 0.95
        )
        ...
        for ha in [x / 100 for x in range(110, 155, 5)]:
            ...
            for _, row in season_df.iterrows():
                ...
                actual = (
                    ...
                    else ("平局" if row["home_goals"] == row["away_goals"] else "客胜")
                )
    ...
    def get_rating_display(self, team: str) -> str:
        ...
        if r:
            return (
                f"攻{r['off']:.2f}/防{r['def']:.2f} | {r['w']}胜{r['d']}平{r['l']}负 | {r['pts']}分"
            )
        return "无数据"
