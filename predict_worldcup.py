...
def main():
    ...
    for m in all_matches:
        ...
        if not model_has_data and market:
            ...
            pred = {
                ...
                "prediction": max(
                    [
                        ("主胜", (1 / market["home"]) / raw),
                        ("平局", (1 / market["draw"]) / raw),
                        ("客胜", (1 / market["away"]) / raw),
                    ],
                    key=lambda x: x[1],
                )[0],
                "confidence": round(
    ...
    for r in sorted(results, key=lambda x: x["date"]):
        ...
        if not r["model_has_data"]:
            lines.append("⚠️ 注: 球队不在国家队评分库中, 使用默认参数预测")
        ...
        if r["value_bets"]:
            ...
            for v in r["value_bets"]:
                lines.append(
                    f"  - {v['outcome']} @ {v['odds']} | 模型概率 {v['model_prob']:.1%} vs 市场 {v['market_prob']:.1%} | 优势 **+{v['edge']:.1%}**"
                )
    ...
    print()
    print(
        f"📊 {'主队':>20s} vs {'客队':<20s} {'模型预测':>6s} {'xG':>8s} {'市场(主/平/客)':>20s} {'价值'}"
    )
    print("-" * 100)
    for r in sorted(results, key=lambda x: x["date"]):
        pred = r["prediction"]
        ...
        print(
            f"  {source_mark} {r['home']:>20s} vs {r['away']:<20s} {pred['prediction']:>6s} "
