...
class ELOSystem:
    ...
    def update(self, home: str, away: str, home_goals: int, away_goals: int, k: Optional[float] = None):
        """根据比赛结果更新 ELO"""
...
def build_training_data(df: pd.DataFrame, elo: ELOSystem) -> pd.DataFrame:
    ...
    for idx, row in df.iterrows():
        ...
        home_pts_last5 = sum(
            3 if g[0] > g[1] else (1 if g[0] == g[1] else 0)
            for g in home_recent
        ) / 5 if home_recent else 1.5
        away_pts_last5 = sum(
            3 if g[0] > g[1] else (1 if g[0] == g[1] else 0)
            for g in away_recent
        ) / 5 if away_recent else 1.2
