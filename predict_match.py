...
from config import FOOTBALL_DATA_KEYS, ODDSPAPI_KEY
...
def find_match(query: str) -> dict:
    ...
    for comp in competitions:
        try:
            ...
            r = requests.get(
                f"{url}?dateFrom={date_from}&dateTo={date_to}", headers=headers, timeout=15
            )
...
def fetch_match_odds(match: dict) -> dict:
    ...
    try:
        ...
        # 拉队名映射
        pr = requests.get(
            f"https://api.oddspapi.io/v4/participants?sportId=10&apiKey={ODDSPAPI_KEY}", timeout=15
        )
...
def generate_report(match, odds, pred, value_bets, snapshots, kickoff_delta) -> str:
    ...
    if match.get("home_score") is not None:
        lines.append(
            f"### 📊 当前比分: {match['home_team']} {match['home_score']} - {match['away_score']} {match['away_team']}"
        )
        lines.append("")
    ...
    if odds:
        ...
        lines.append(
            f"| 市场概率 | {(1 / odds['home'] / raw):.1%} | {(1 / odds['draw'] / raw):.1%} | {(1 / odds['away'] / raw):.1%} |"
        )
    ...
    if pred.get("home_xg", 0) > 0:
        lines.append(
            f"🔮 **预测: {pred['prediction']}** ({pred['confidence']:.0%}) | xG: {pred['home_xg']}-{pred['away_xg']}"
        )
    else:
    ...
    if value_bets:
        ...
        if real_v:
            ...
            for v in sorted(real_v, key=lambda x: -x["edge"]):
                ...
                lines.append(
                    f"| {v['outcome']} | {v['odds']} | {v['market_prob']:.1%} | {v['model_prob']:.1%} | {mark} +{v['edge']:.1%} |"
                )
    ...
    if snapshots:
        ...
        for s in snapshots:
            ...
            if o:
                lines.append(
                    f"| {ts} | {delta_str} | {o.get('home', '?')} | {o.get('draw', '?')} | {o.get('away', '?')} | {p.get('prediction', '?')} | {p.get('confidence', 0):.0%} |"
                )
            else:
                lines.append(
                    f"| {ts} | {delta_str} | — | — | — | {p.get('prediction', '?')} | {p.get('confidence', 0):.0%} |"
                )
    ...
    if len(snapshots) >= 3:
        ...
        if stable:
            lines.append("- ✅ 赔率走势稳定, 预测可靠性高")
        else:
            lines.append("- ⚠️ 赔率波动较大, 注意市场情绪变化")
    ...
    if value_bets:
        ...
        if best_value["edge"] > 0.05:
            lines.append(
                f"- 💡 市场可能低估了 **{best_value['outcome']}** (优势 +{best_value['edge']:.1%})"
            )

    lines.append("")
    lines.append("---")
    lines.append(
        f'> 🤖 自动生成 | 下次运行 `python3 predict_match.py "{home} vs {away}"` 可追加新时间点'
    )
...
def main():
    ...
    if args.id:
        ...
        headers = {"X-Auth-Token": FD_KEY}
        r = requests.get(
            f"https://api.football-data.org/v4/matches/{args.id}", headers=headers, timeout=10
        )
        if r.status_code != 200:
    ...
    from engine.five_step import generate_full_report as gen_five_step_report
    ...
    print(
        f"  {'综合':>6s}: {five_step_result['total_score']:.0%} → {five_step_result['suggested_action']}"
    )
    value_bets = []
    if odds:
        raw = 1 / odds["home"] + 1 / odds["draw"] + 1 / odds["away"]
        for outcome, mk_o, p_key in [
            ("主胜", odds["home"], "p_home"),
            ("平局", odds["draw"], "p_draw"),
            ("客胜", odds["away"], "p_away"),
        ]:
        ...
        if value_bets:
            ...
            for v in sorted(value_bets, key=lambda x: -x["edge"]):
                print(
                    f"   {v['outcome']} @ {v['odds']} | 模型 {v['model_prob']:.1%} vs 市场 {v['market_prob']:.1%} | +{v['edge']:.1%}"
                )
