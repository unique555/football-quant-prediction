...
def build_team_strength(fixtures: list[dict]) -> dict:
    ...
    for fx in fixtures:
        ...
        teams[home]["gf"] += hg; teams[home]["ga"] += ag; teams[home]["gp"] += 1
        teams[away]["gf"] += ag; teams[away]["ga"] += hg; teams[away]["gp"] += 1
        if hg > ag: teams[home]["w"] += 1; teams[away]["l"] += 1
        elif hg == ag: teams[home]["d"] += 1; teams[away]["d"] += 1
        else: teams[away]["w"] += 1; teams[home]["l"] += 1
...
def fuzzy_match(op_name: str, ratings: dict) -> Optional[str]:
    ...
    for rname in ratings:
        ...
        if op_lower == r_lower: return rname
        if op_lower in r_lower or r_lower in op_lower: return rname
...
def run_backtest(fixtures: list[dict], ratings: dict) -> dict:
    ...
    for fx in fixtures:
        if fx["home_goals"] is None or fx["away_goals"] is None: continue
        (h_odds, d_odds, a_odds), ph, pd, pa = synthetic_odds(
            fx["home_team"], fx["away_team"], ratings)
        actual = "home" if fx["home_goals"] > fx["away_goals"] else (
        ...
        rows.append({"date": fx["date"], "home_team": fx["home_team"],
                     "away_team": fx["away_team"], "home_goals": fx["home_goals"],
                     "away_goals": fx["away_goals"], "home_odds": h_odds,
                     "draw_odds": d_odds, "away_odds": a_odds, "actual": actual, "pred": pred})
    n = len(rows)
    return {
        "total": n,
...
def generate_report(all_data: dict) -> str:
    ...
    lines = [
        ...
        f"",
        f"> 生成时间: {now}  ",
        f"> 数据源: OddsPapi (Pinnacle) + API-Football  ",
        f"> 模型: Poisson 分布 × 球队实力评分  ",
    ...
    for name, d in all_data.items():
        ...
        lines.append(f"| {name} | {status} | {d['current_matches']} | {d['historical_fixtures']} | {acc:.1%} |")
    lines.append("")
    ...
    if has_current:
        ...
        for league_name, d in has_current:
            ...
            for m in d["matches"]:
                ...
                mph, mpd, mpa = (1/m["home_odds"])/raw, (1/m["draw_odds"])/raw, (1/m["away_odds"])/raw
                best = max([("主胜", pred["p_home"]), ("平局", pred["p_draw"]), ("客胜", pred["p_away"])], key=lambda x:x[1])

                lines.append(f"**{m['home_team']} vs {m['away_team']}**  ")
                lines.append(f"⏰ {m['start_time'][:16].replace('T',' ')}  ")
                lines.append(f"")
                lines.append(f"|  | 主胜 | 平局 | 客胜 |")
                lines.append(f"|--|------|------|------|")
                ...
                lines.append(f"")
                lines.append(f"🔮 预测: **{best[0]}** ({best[1]:.1%}) | xG: {pred['home_xg']}-{pred['away_xg']} | 返还率: {1/raw:.1%}")

                # 价值投注标记
                edges = []
                if pred["p_home"]/mph - 1 > 0.05: edges.append(f"主胜 (edge +{pred['p_home']/mph-1:.1%})")
                if pred["p_draw"]/mpd - 1 > 0.05: edges.append(f"平局 (edge +{pred['p_draw']/mpd-1:.1%})")
                if pred["p_away"]/mpa - 1 > 0.05: edges.append(f"客胜 (edge +{pred['p_away']/mpa-1:.1%})")
                if edges:
    ...
    for name, d in all_data.items():
        bt = d.get("backtest", {})
        if bt:
            lines.append(f"| {name} | {bt['total']} | {bt['accuracy']:.1%} | {bt['brier']:.4f} | {bt['home_pct']:.1%} | {bt['draw_pct']:.1%} | {bt['away_pct']:.1%} |")
    lines.append("")
...
def main():
        ...
        if fixtures:
            ...
            if current_matches:
                for m in current_matches:
                    ...
                    pred = predict_match(
                        hm if hm else m["home_team"],
                        am if am else m["away_team"],
                        ratings
                    )
                    m["prediction"] = pred
                    m["home_rating_match"] = hm
                    m["away_rating_match"] = am
                matched = sum(1 for m in current_matches if m.get("home_rating_match") and m.get("away_rating_match"))
                print(f"   🎯 队名匹配: {matched}/{len(current_matches)}")
                league_result["matches"] = current_matches
        else:
            print(f"   ⚠️ 无历史数据，跳过评分/预测")
    ...
    for name, d in all_data.items():
        ...
        print(f"  {has_odds} {name:6s}: 当前{d['current_matches']:2d}场 | 历史{d['historical_fixtures']:3d}场 | 回测{bt.get('accuracy',0):.0%}")
    print()
