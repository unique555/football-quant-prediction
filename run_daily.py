...
from utils import adjust_inplay as _adjust_inplay
...
def generate_md(
    matches: list, live_matches: list, model, today_str: str, retrain_count: int
) -> str:
    lines = [
        f"# ⚽ 每日预测报告 — {today_str}",
        "",
    ...
    if live_matches:
        ...
        for m in live_matches:
            ...
            lines.append(
                f"  xG: {pred.get('home_xg', '?')}-{pred.get('away_xg', '?')} | 模型预测: {pred.get('prediction', '?')} ({pred.get('confidence', 0):.0%})"
            )
            lines.append("")
        lines.append("")
    ...
    if matches:
        ...
        for m in matches:
            ...
            if mk:
                ...
                lines.append(f"| 市场赔率 | {mk['home']} | {mk['draw']} | {mk['away']} |")
                lines.append(
            ...
            if values:
                ...
                if real_value:
                    ...
                    for v in real_value:
                        lines.append(
                            f"  - {v['outcome']} @ {v['odds']} (模型概率 {v['model_prob']:.1%}, 优势 +{v['edge']:.1%})"
                        )
    ...
    lines.append("---")
    lines.append(
        f"> 🤖 自动生成于 {datetime.now().strftime('%Y-%m-%d %H:%M')} | 主场优势系数: {model.home_advantage}"
    )
...
def main():
    ...
    if not model.ratings or model.last_trained is None:
        df = pd.read_csv(
            "/workspace/football-quant-prediction/data/football_data_co_uk.csv", low_memory=False
        )
    ...
    for m in all_selected:
        ...
        if tid and code in odds_cache:
            ...
            for p1_id, o in odds_for_league.items():
                ...
                if p1_name and (
                    m["home_team"].lower() in p1_name.lower()
                    or p1_name.lower() in m["home_team"].lower()
                ):
                    market_odds = o
                    break
        ...
        if market_odds:
            ...
            m["payout"] = 1 / raw
            for outcome, mk_o, mp_key in [
                ("主胜", market_odds["home"], "p_home"),
                ("平局", market_odds["draw"], "p_draw"),
                ("客胜", market_odds["away"], "p_away"),
            ]:
                mk_prob = (1 / mk_o) / raw
