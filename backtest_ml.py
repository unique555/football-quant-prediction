...
from engine.orchestrator.pipeline import FinalVerdict, run_full_pipeline
...
def generate_odds_row(row) -> list[BookmakerSnapshot]:
    ...
    for name in bookmaker_names:
        ...
        odds_a = max(
            1.05, (1.0 / (max(0.03, 1.0 - (p_home + noise_h) - (p_draw + noise_d)))) * (1 / margin)
        )
...
def run_ml_backtest(test_df: pd.DataFrame, model, scaler, outcome_map):
    ...
    for i, (_, row) in enumerate(test_df.iterrows()):
        ...
        ah, ad, aw = av[actual]
        bs = (
            (report.final_home_prob - ah) ** 2
            + (report.final_draw_prob - ad) ** 2
            + (report.final_away_prob - aw) ** 2
        ) / 3
        brier_sum += bs
    ...
    accs = {
        ver: (sum(by_verdict[ver]) / len(by_verdict[ver]) if by_verdict[ver] else 0)
    ...
    if has_h and has_l and accs["high_confidence"] > accs["low_confidence"]:
        print(
            f"  ✅ 高置信度 ({accs['high_confidence']:.1%}) > 低置信度 ({accs['low_confidence']:.1%})"
        )
        print("     系统正确识别了高质量信号")
    print()
