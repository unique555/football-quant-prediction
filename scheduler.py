...
from notify import send, status as notify_status
...
def run_task(task_name: str, config: dict, state: dict) -> bool:
    ...
    print(f"{'='*50}")
    try:
        result = subprocess.run(config["cmd"], shell=True, cwd="/workspace/football-quant-prediction",
        ...
        send(f"{status_icon} {config['desc']}: {'完成' if success else '失败'} | {datetime.now().strftime('%H:%M')}")
        ...
        try:
            ...
            for m in r.json().get("matches", []):
                ...
                if mid not in monitored:
                    monitored[mid] = {"last_checkpoint": None, "team": f"{m['homeTeam']['name']} vs {m['awayTeam']['name']}"}
                
                # 检查是否到了下一个 checkpoint
                for cp in sorted(checkpoints):
                    if mins_to_kickoff <= cp and (monitored[mid]["last_checkpoint"] is None or monitored[mid]["last_checkpoint"] > cp):
                        print(f"\n⏰ [{datetime.now().strftime('%H:%M')}] {monitored[mid]['team']} → checkpoint {cp}min")
                        subprocess.run(
                            f"python3 predict_match.py --id {mid}",
                            shell=True, cwd="/workspace/football-quant-prediction",
                            timeout=60
                        )
            ...
            # 清理过期监控
            expired = [mid for mid, data in monitored.items() if (kickoff := datetime.fromisoformat(
                next((m["utcDate"] for m in r.json().get("matches", []) if m["id"] == mid), "2000-01-01T00:00:00Z"
            ).replace("Z", "+00:00"))) and (kickoff - today).total_seconds() / 60 < -120]
            for mid in expired:
                del monitored[mid]
...
def main():
    ...
    if args.watch_match:
        ...
        while True:
            subprocess.run(f"python3 predict_match.py --id {args.watch_match}", shell=True,
                         cwd="/workspace/football-quant-prediction", timeout=60)
            print(f"  ⏳ 15分钟后再次检查...")
            time.sleep(900)
