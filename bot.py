...
from engine.five_step import MatchAnalyzer, generate_full_report
...
def cmd_predict(chat_id: int, query: str):
    ...
    try:
        ...
        for item in odds_r.json():
            ...
            if match["homeTeam"]["name"].lower() in p1.lower() and match["awayTeam"]["name"].lower() in p2.lower():
                bm = item["bookmakerOdds"]["pinnacle"]
                mkt = bm["markets"]["101"]["outcomes"]
                odds = {"home": float(mkt["101"]["players"]["0"]["price"]),
    ...
    # 五步分析
    match_obj = {
        "match_id": match["id"], "home_team": match["homeTeam"]["name"],
        "away_team": match["awayTeam"]["name"], "date": match["utcDate"],
        ...
        "market_odds": odds, "tournament_id": 16,
    }
    ...
    text += f"② 共识: {consensus.get('consensus_level')} ({consensus.get('consensus_score',0):.0%})\n"
...
def cmd_today(chat_id: int):
    ...
    for m in all_matches[:10]:  # 最多10场
        mo = {
            ...
            "competition": "WC", "competition_name": m.get("competition", {}).get("name", ""),
            "stage": m.get("stage", ""), "status": m.get("status", ""),
            "market_odds": None, "tournament_id": 16,
        }
...
def cmd_help(chat_id: int):
    send_msg(chat_id, """🤖 Football-Quant 机器人
    ...
    while True:
        try:
            ...
            for upd in updates:
                ...
                if text.startswith("/分析") or text.startswith("／分析"):
                ...
                elif text.startswith("/今日") or text.startswith("／今日") or text == "今日" or text == "竞彩":
                    try:
                        cmd_today(chat_id)
                    except Exception as e:
                    ...
                    send_msg(chat_id, "👋 Football-Quant 已就绪\n\n发送 /帮助 查看可用命令\n发送 /今日 查看今日竞彩")
