...
def generate_weekly_md(week_start: str, week_end: str, results: list, opt_result: dict) -> str:
    week_num = datetime.strptime(week_start, "%Y-%m-%d").strftime("%Y-W%W")

    total_matches = sum(r["total"] for r in results)
    avg_acc = (
        sum(r["accuracy"] * r["total"] for r in results) / total_matches if total_matches else 0
    )
    avg_brier = (
        sum(r["brier"] * r["total"] for r in results) / total_matches if total_matches else 0
    )
    ...
    for r in results:
        trend = (
            "📈" if r["accuracy"] > avg_acc else ("📉" if r["accuracy"] < avg_acc - 0.05 else "➡️")
        )
        lines.append(
            f"| {r['date']} | {r['total']} | {trend} {r['accuracy']:.0%} | {r['brier']:.4f} |"
        )
...
def main():
    ...
    summary = {
        ...
        "end": week_end,
        "total_matches": sum(r["total"] for r in results),
        "accuracy": sum(r["accuracy"] * r["total"] for r in results)
