...
from engine.config.settings import engine_config, ClassifierConfig
...
# 德比关键词
DERBY_KEYWORDS = [
    "德比", "derby", "classico", "classic",
    "德比战", "同城", "国家德比", "rivalry",
    "曼市德比", "北伦敦德比", "马德里德比", "米兰德比",
]
...
def classify_match(
    ...
    if is_derby:
        return ClassificationResult(
            ...
            tsi_gap=tsi_gap, favorite_side=favorite,
    ...
    risk_flags, form_risk = _detect_upset_flags_v2(
        abs_gap=abs_gap, fav_handicap=fav_handicap,
        odds_movement=odds_movement,
        fav_absence=fav_absence, dog_absence=dog_absence,
        fav_form=fav_form, dog_form=dog_form, cfg=cfg,
    )

    # ---- 5. 价值陷阱检测 ----
    trap_flags = _detect_value_trap_v2(
        abs_gap=abs_gap, fav_handicap=fav_handicap,
        initial_odds=initial_odds, sig=sig, cfg=cfg,
    )
    ...
    confidence = _compute_confidence_v2(
        matched_type, abs_gap, fav_handicap,
        risk_flags, trap_flags, decay, cfg,
    )
    ...
    return ClassificationResult(
        ...
        reason=_generate_reason_v2(matched_type, favorite, abs_gap, risk_flags, trap_flags, form_risk),
        subtype_flags=risk_flags + trap_flags + ([form_risk] if form_risk else []),
        tsi_home=tsi_home, tsi_away=tsi_away, tsi_gap=tsi_gap,
...
def _detect_upset_flags_v2(
    abs_gap: float, fav_handicap: Optional[float],
    odds_movement: Optional[str],
    fav_absence: int, dog_absence: int,
    fav_form: float, dog_form: float,
    cfg: ClassifierConfig,
    ...
    # 强队浅盘
    if abs_gap >= cfg.upset_tsi_threshold and fav_handicap is not None and fav_handicap > cfg.upset_shallow_handicap:
    ...
    # 单边受热 + 浅盘
    if odds_movement == "steaming" and fav_handicap is not None and fav_handicap > cfg.upset_shallow_handicap:
...
def _detect_value_trap_v2(
    ...
    # 低赔 + 深盘 + 实力不足 = 陷阱
    if fav_odds < cfg.trap_low_odds and fav_handicap <= cfg.trap_handicap_threshold and abs_gap < cfg.trap_tsi_gap_max:
...
def _detect_phase(
    ...
    if progress <= 0.15:
        ...
        # 最后 8 轮: 可能争冠/保级
        if (home_pos <= 3 or away_pos <= 3) and (home_pos >= total_rounds - 6 or away_pos >= total_rounds - 6):
            phase = MatchPhase.CRUNCH
        elif abs(home_pos - away_pos) <= 3 and (home_pos >= total_rounds - 6 or away_pos >= total_rounds - 6):
...
def _compute_confidence_v2(
    match_type: MatchType, abs_gap: float,
    fav_handicap: Optional[float],
    risk_flags: list[str], trap_flags: list[str],
    decay: float, cfg: ClassifierConfig,
) -> float:
    """增强版置信度"""
    base = 0.70

    if abs_gap > 15:  base += 0.15
    if fav_handicap is not None and abs(fav_handicap) > 0.5:  base += 0.10
    if match_type == MatchType.DERBY_SPECIAL:  base -= cfg.derby_confidence_penalty
    if risk_flags:  base -= 0.05 * len(risk_flags)
    if trap_flags:  base -= 0.10 * len(trap_flags)
    base *= decay
...
def _generate_reason_v2(
    match_type: MatchType, favorite: str, abs_gap: float,
    risk_flags: list[str], trap_flags: list[str],
