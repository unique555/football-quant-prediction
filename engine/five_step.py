...
class MatchProfiler:
    ...
    def profile(match: dict, model_pred: dict, odds: Optional[dict]) -> dict:
        ...
        rating_gap = 0
        if hr and ar:
            rating_gap = (hr.get("off", 1) - hr.get("def", 1)) - (ar.get("off", 1) - ar.get("def", 1))
        ...
        if odds:
            if odds["home"] < 1.5:
                odds_type = MatchProfiler.STRONG_FAV; fav_side = "主队"
            elif odds["home"] < 1.8:
                odds_type = MatchProfiler.CLEAR_FAV; fav_side = "主队"
            elif odds["home"] < 2.3:
                odds_type = MatchProfiler.MODERATE; fav_side = "主队" if odds["home"] < odds["away"] else "客队"
            elif odds["away"] < 2.3:
                odds_type = MatchProfiler.MODERATE; fav_side = "客队"
            elif odds["away"] < 1.8:
                odds_type = MatchProfiler.CLEAR_FAV; fav_side = "客队"
            elif odds["away"] < 1.5:
                odds_type = MatchProfiler.STRONG_FAV; fav_side = "客队"
        ...
        goal_type = "小球 (<2.5)" if xg_total < 2.3 else ("大球 (>2.5)" if xg_total > 2.8 else "中等进球")
        
        # 赛事权重
        stage = match.get("stage", "")
        tournament_weight = 1.5 if "FINAL" in stage else (1.3 if "SEMI" in stage else (1.1 if "QUARTER" in stage else 1.0))
        ...
        return {
            ...
            "volatility": "高" if odds_type in (MatchProfiler.BALANCE, MatchProfiler.UPSET_RISK) else ("中" if odds_type == MatchProfiler.MODERATE else "低"),
            "confidence_zone": "可重注" if odds_type == MatchProfiler.STRONG_FAV else ("稳健" if odds_type == MatchProfiler.CLEAR_FAV else ("谨慎" if odds_type == MatchProfiler.MODERATE else "高风险")),
        }
...
class ConsensusAnalyzer:
    ...
    def analyze(match: dict, odds_api_key: str) -> dict:
            ...
            try:
                ...
                for item in r.json():
                    ...
                    try:
                        ...
                        raw = 1/h + 1/d + 1/a
                        p_h = (1/h)/raw; p_d = (1/d)/raw; p_a = (1/a)/raw
                        best = max([("home", p_h), ("draw", p_d), ("away", p_a)], key=lambda x: x[1])
        ...
        level = "高度一致" if consensus_score >= 0.8 else ("多数一致" if consensus_score >= 0.6 else ("分歧较大" if consensus_score >= 0.4 else "严重分歧"))
...
class TechnicalValidator:
    ...
    @staticmethod
    def validate(model_pred: dict, market_odds: Optional[dict], 
                 consensus: dict, profile: dict) -> dict:
        checks = []
        score = 0  # 通过的检查数
        ...
        if market_odds:
            ...
            clv_signals = []
            for side, fair, mk in [("主", fair_h, market_odds["home"]), 
                                    ("平", fair_d, market_odds["draw"]),
                                    ("客", fair_a, market_odds["away"])]:
                clv = (fair - mk) / mk * 100
                if clv > 5: 
                    clv_signals.append(f"{side}胜被低估 {clv:.1f}%")
                    if side == "主" and model_pred["prediction"] == "主胜": score += 1
                    if side == "客" and model_pred["prediction"] == "客胜": score += 1
                elif clv < -15:
                    clv_signals.append(f"{side}胜被高估 {abs(clv):.1f}%")
            
            checks.append({"name": "CLV检验", "signals": clv_signals, "passed": len(clv_signals) > 0})
        ...
        if consensus_ok:
            ...
            checks.append({"name": "机构共识", "passed": True, "aligned": aligned, 
        ...
        if market_odds:
            ...
            checks.append({"name": "赔率结构", "passed": structure_ok, 
                          "detail": f"返还率 {payout:.1%}"})
        
        # 4) 波动性检查
        vol_ok = profile.get("volatility") == "低" or (profile.get("volatility") == "中" and model_pred["confidence"] > 0.4)
        if vol_ok: score += 1
        checks.append({"name": "波动性", "passed": vol_ok, "detail": profile.get("volatility", "?")})
        
        return {
            "checks": checks,
            "score": score,
            "max_score": len(checks),
            "verdict": "通过" if score >= len(checks) * 0.6 else ("部分通过" if score >= len(checks) * 0.3 else "不通过"),
        }
    ...
    def test(model_pred: dict, market_odds: Optional[dict], profile: dict) -> dict:
        ...
        for side, mk_odds, p_key, outcome in [
            ...
            results[outcome] = {
                ...
                "ev": round(ev, 4),
                "recommend": "重注" if kelly_half > 0.05 else ("轻注" if kelly_half > 0.02 else ("观望" if kelly_half > -0.02 else "回避")),
            }
        ...
        return {
            ...
            "anomalies": anomalies,
            "best_value": max(results.items(), key=lambda x: x[1]["kelly_half"]) if results else None,
        }
...
def generate_full_report(match: dict, result: dict) -> str:
    ...
    lines.append(f"- 进球预期: {profile.get('goal_profile','?')} (xG {profile.get('xg_total','?')})")
    ...
    lines.append(f"- 共识度: **{consensus.get('consensus_level','?')}** ({consensus.get('consensus_score',0):.0%})")
    bd = consensus.get("breakdown", {})
    if bd:
        lines.append(f"- 方向分布: 主{bd.get('home',0)} / 平{bd.get('draw',0)} / 客{bd.get('away',0)}")
    ...
    # ③
    lines.append("## ③ 技术验证")
    lines.append(f"- 综合判定: {tech.get('verdict','?')} ({tech.get('score',0)}/{tech.get('max_score',0)}项)")
    ...
    if ka:
        ...
        for outcome, v in ka.items():
            lines.append(f"| {outcome} | {v['kelly_full']:.3f} | {v['kelly_half']:.3f} | {v['ev']:+.3f} | {v['recommend']} |")
    ...
    lines.append("| 步骤 | 评分 |")
    lines.append("|------|------|")
    step_names = {"profile": "①比赛画像", "consensus": "②机构共识", "technical": "③技术验证", "pricing": "④定价检验"}
    ...
    lines.append("")
    lines.append("---")
    lines.append(f"> 🤖 五步分析引擎 | 仅供参考, 不构成投注建议")
